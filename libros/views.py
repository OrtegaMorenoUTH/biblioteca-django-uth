...
def lista_libros(request):
    """Listado de todos los libros con filtros y paginación"""
    libros_qs = Libro.objects.select_related('autor', 'editorial').prefetch_related('categorias')
    ...
    if categoria_id:
        libros_qs = libros_qs.filter(categorias__id=categoria_id)
    if autor_id:
        libros_qs = libros_qs.filter(autor__id=autor_id)
    if estado:
        libros_qs = libros_qs.filter(estado=estado)
    if busqueda:
    ...
    per_page = 4
    paginator = Paginator(libros_qs, per_page)
    page_number = request.GET.get('page', 1)
    try:
        libros_page = paginator.page(page_number)
    except PageNotAnInteger:
        libros_page = paginator.page(1)
    except EmptyPage:
        libros_page = paginator.page(paginator.num_pages)

    # Construir querystring con los GET params excepto page (para mantener filtros al navegar)
    ...
    context = {
        'libros': libros_page,              # en plantilla se itera igual: for libro in libros
...
def detalle_libro(request, id):
    ...
    ).exclude(id=libro.id).distinct()[:3]
...
def lista_autores(request):
    """Listado de todos los autores con paginación"""
    autores_qs = Autor.objects.annotate(
    ...
    busqueda = request.GET.get('q')
    if busqueda:
        autores_qs = autores_qs.filter(
            Q(nombre__icontains=busqueda) |
            Q(pais_origen__icontains=busqueda)
        )

    total_autores = autores_qs.count()

    # Paginación: 5 autores por página
    per_page = 3
    paginator = Paginator(autores_qs, per_page)
    page_number = request.GET.get('page', 1)
    try:
        autores_page = paginator.page(page_number)
    except PageNotAnInteger:
        autores_page = paginator.page(1)
    except EmptyPage:
        autores_page = paginator.page(paginator.num_pages)
    ...
    context = {
        'autores': autores_page,
        'page_obj': autores_page,
        'paginator': paginator,
        'is_paginated': paginator.num_pages > 1,
        'busqueda': busqueda,
        'total_autores': total_autores,
...
def detalle_autor(request, id):
    """Detalles de un autor y sus libros (con paginación en la lista de libros)"""
    autor = get_object_or_404(Autor, id=id)
    libros_qs = Libro.objects.filter(autor=autor).prefetch_related('categorias')
    
    # Estadísticas del autor
    total_libros = libros_qs.count()
    calificacion_promedio = libros_qs.aggregate(Avg('calificacion'))['calificacion__avg'] or 0

    # Paginación de los libros del autor
    per_page = 4
    paginator = Paginator(libros_qs, per_page)
    page_number = request.GET.get('page', 1)
    try:
